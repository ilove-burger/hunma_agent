...
def blog_cell_glyph(cell: dict[str, Any] | None) -> str:
    if cell is None:
        return "⚪ N/A"
    match = re.search(r"=(\w+) expected", cell["marker_summary"])
    marker_word = match.group(1) if match else cell["marker_summary"]
    glyph = "🟢" if cell["status"] == "PASS" else "🔴"
    return f"{glyph} {marker_word}"


def write_blog_report_export(
    artifacts_dir: Path, comparison_name: str, status: str, summary: dict[str, Any]
) -> dict[str, str]:
    ...
    for row in summary_table:
        ...
        cells.extend(blog_cell_glyph(row.get(role)) for role in roles)
        lines.append("| " + " | ".join(cells) + " |")
    lines.extend(
        [
            "",
            "🟢 = 관찰값이 기대값과 일치, 🔴 = 불일치, ⚪ N/A = 해당 target에 적용할 수 없는 lifecycle.",
        ]
    )

    report_path = artifacts_dir / "report.md"
    report_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return {"blog_report_markdown": str(report_path)}
...
def run_comparison(
    *,
    ...
    try:
        ...
        exports.update(write_blog_report_export(artifacts_dir, comparison_name, status, summary))
        result = {
            "schema_version": 1,
            "comparison": comparison_name,
            "status": status,
